...
// Launched via `agent.exe --config-ui`; edits config.json so production
// users can set the server address without hand-editing JSON.
...
ConfigUi* g_cfg = nullptr;
HFONT g_font = nullptr;
WNDPROC g_pwd_old_proc = nullptr;
float g_scale = 1.0f;
...
HWND make_label(HWND parent, const wchar_t* text, int x, int y, int w) {
    HWND h = CreateWindowExW(0, L"STATIC", text, WS_CHILD | WS_VISIBLE,
                             x, y, w, px(kLabelH), parent, nullptr, nullptr,
                             nullptr);
    SendMessageW(h, WM_SETFONT, reinterpret_cast<WPARAM>(g_font), TRUE);
    return h;
}

HWND make_edit(HWND parent, int id, const std::wstring& text, int x, int y,
               bool password) {
    ...
    HWND h = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", text.c_str(), style,
                             x, y, px(kFieldW), px(kFieldH), parent,
...
HWND make_button(HWND parent, int id, const wchar_t* text, int x, int y, int w,
                 int height = 30) {
    HWND h = CreateWindowExW(0, L"BUTTON", text, WS_CHILD | WS_VISIBLE | WS_TABSTOP,
                             x, y, px(w), px(height), parent,
                             reinterpret_cast<HMENU>(static_cast<INT_PTR>(id)),
                             nullptr, nullptr);
    SendMessageW(h, WM_SETFONT, reinterpret_cast<WPARAM>(g_font), TRUE);
    return h;
...
// so the result matches what the real agent would experience.
std::wstring test_register(const std::string& host, int port,
    ...
    if (rc == SOCKET_ERROR) {
        if (WSAGetLastError() != WSAEWOULDBLOCK) {
            ...
            WSACleanup();
            return L"连接失败：" +
                   wide("错误码 " + std::to_string(WSAGetLastError())) +
...
LRESULT CALLBACK pwd_edit_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    if (msg == WM_KEYDOWN && wp == VK_RETURN) {
        ...
        SendMessageW(parent, WM_COMMAND, IDC_BTN_SAVE, 0);
        return 0;
    }
    return CallWindowProcW(g_pwd_old_proc, hwnd, msg, wp, lp);
}
...
void on_save(HWND hwnd) {
    ...
    g_cfg->server_ip = host;
    g_cfg->server_port = std::stoi(port_s);
    g_cfg->secret_key = utf8(GetDlgItem(hwnd, IDC_ED_KEY));
    g_cfg->device_name = utf8(GetDlgItem(hwnd, IDC_ED_NAME));
    g_cfg->control_password = utf8(GetDlgItem(hwnd, IDC_ED_PWD));

    config::ClientConfig disk;
    disk.server_ip = g_cfg->server_ip;
    disk.server_port = g_cfg->server_port;
    disk.secret_key = g_cfg->secret_key;
    disk.device_name = g_cfg->device_name;
    disk.control_password = g_cfg->control_password;
    if (!config::ClientConfig::save(disk, g_cfg->config_path)) {
        set_status(hwnd, L"保存失败：无法写入 " + wide(g_cfg->config_path) +
                         L"（权限不足或被占用？）");
        return;
    }
    g_cfg->saved = true;
...
LRESULT CALLBACK wnd_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
        case WM_CREATE: {
            ...
            int y = px(kMargin);
            make_label(hwnd, L"服务器地址（控制端电脑的 IP）", px(kMargin), y,
                       px(kFieldW));
            y += px(kLabelH) + px(2);
            make_edit(hwnd, IDC_ED_IP, wide(g_cfg ? g_cfg->server_ip : ""),
                      px(kMargin), y, false);
            y += px(kFieldH) + px(kGap);

            make_label(hwnd, L"服务器端口", px(kMargin), y, px(kFieldW));
            y += px(kLabelH) + px(2);
            make_edit(hwnd, IDC_ED_PORT,
                      wide(std::to_string(g_cfg ? g_cfg->server_port : 7500)),
                      px(kMargin), y, false);
            y += px(kFieldH) + px(kGap);

            make_label(hwnd, L"连接密钥（必须与服务端 secret_key 完全一致）",
                       px(kMargin), y, px(kFieldW));
            y += px(kLabelH) + px(2);
            make_edit(hwnd, IDC_ED_KEY, wide(g_cfg ? g_cfg->secret_key : ""),
                      px(kMargin), y, false);
            y += px(kFieldH) + px(kGap);

            make_label(hwnd, L"设备名称（留空自动使用计算机名）", px(kMargin), y,
                       px(kFieldW));
            y += px(kLabelH) + px(2);
            make_edit(hwnd, IDC_ED_NAME, wide(g_cfg ? g_cfg->device_name : ""),
                      px(kMargin), y, false);
            y += px(kFieldH) + px(kGap);

            make_label(hwnd, L"控制密码（远程操控前的二次验证，可留空）",
                       px(kMargin), y, px(kFieldW));
            y += px(kLabelH) + px(2);
            HWND pwd = make_edit(hwnd, IDC_ED_PWD,
                                 wide(g_cfg ? g_cfg->control_password : ""),
                                 px(kMargin), y, true);
            g_pwd_old_proc = reinterpret_cast<WNDPROC>(SetWindowLongPtrW(
                pwd, GWLP_WNDPROC,
                reinterpret_cast<LONG_PTR>(pwd_edit_proc)));
            y += px(kFieldH) + px(kGap) + px(4);

            make_button(hwnd, IDC_BTN_TEST, L"测试连接并注册", px(kMargin), y,
                        140);
            y += px(30) + px(kGap);

            HWND status = CreateWindowExW(
                0, L"STATIC", L"", WS_CHILD | WS_VISIBLE | SS_LEFT,
                px(kMargin), y, px(kFieldW), px(66), hwnd,
                reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_ST_STATUS)),
                nullptr, nullptr);
            ...
            HWND file_label = CreateWindowExW(
                0, L"STATIC",
                (L"配置文件：" +
                 wide(g_cfg ? g_cfg->config_path : std::string()))
                    .c_str(),
                WS_CHILD | WS_VISIBLE | SS_LEFT | SS_PATHELLIPSIS,
                px(kMargin), y, px(kFieldW), px(kLabelH), hwnd,
                reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_ST_FILE)),
                nullptr, nullptr);
            SendMessageW(file_label, WM_SETFONT,
                         reinterpret_cast<WPARAM>(g_font), TRUE);

            // Multiline status text needs a mono-ish readable font.
...
bool run_config_gui(ConfigUi& cfg) {
    ...
    const wchar_t* cls = L"MyRemoteConfigWnd";
    WNDCLASSW wc{};
    ...
    RECT rc{0, 0, px(kClientW), px(kClientH)};
