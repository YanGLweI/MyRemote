...
- M21-8 真机复测：**TEST-WIN** 装 1.0.4，三条症状用户反馈通过——**口头确认，没带回日志/截图**，台账按"报障机复测通过"记。**YEUNG** 覆盖安装 1.0.4（agent + control_server 都实测 FileVersion 1.0.4，服务 Running/Automatic），当场留下三条硬证据：`tray-2.log` 的 `Tray proxy config: C:\MyRemote\Agent\config.json (found)`（M21-1）、`config.json` 与 `unins000.dat` 同秒被原地重写且 `control_password` 存活（M21-6）、`--service-state` 的 `host:` 无 `STALE` 而 `host.status` 就在几秒前（M21-5 的反面）。
...
- **仍无现场证据**：只剩 M20 那七条自愈判据（假宿主故障注入、explorer 重启后仍弹菜单、退出无残留、僵死代理 ≤10s 自愈、暂停/退出/隐藏语义、覆盖安装的瞬间窗口期）。两台现在都是 1.0.4，随时可做，但 ① 那条要停服务跑假宿主、需要人在机器前。
