...
>
> 其余徽章：`〔服务〕`由 MyRemoteAgent 服务托管；`〔登录界面〕`对端此刻正停在登录/凭据界面，可以直接远程输密码；`〔非控制台〕`该会话此刻没接在物理显示器上（典型原因是有人正用 RDP 操作这台机器）——隧道不会因此断开，RDP 一放手画面就自己回来，无需人工干预。想主动回到登录界面输密码，点“返回登录界面”（锁定工作站）。注意 **Ctrl+Alt+Del 无法被注入**：若该机策略要求先按 Ctrl+Alt+Del 才出现登录框（`DisableCAD=0`），这一页远程点不动，请将该值设为 1。
