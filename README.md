...
装了服务的机器过去**没有任何可见痕迹**。现在被控端会在通知区域放一枚图标（宿主守在 `Winsta0\Default` 桌面，所以停在登录界面或按了 Win+L 时它自然看不见，解锁就回来）。右键菜单四项的语义是刻意分开的：
...
- 登录界面相关（M14）的能力边界：
  ...
  - 装了服务之后再点托盘里的“打开配置”，是**SYSTEM 替你写这份配置**：这是"坐在机器前的人要能控制本机客户端"换来的结果，于是本机有限权限用户能借一个点开的窗口改到 SYSTEM 隧道的服务器地址与控制密码（写的是同一个文件，权限却比他自己高）。想彻底关掉这个入口就隐藏托盘图标，配置地址仍可由管理员直接改文件
  - 托盘图标是 SYSTEM 画在用户桌面上的 UI：同一会话里的任何进程都能画出一枚一模一样的图标和菜单，所以它只能当作现场信息与操作入口，**不能当作"这台机器正被谁控制"的权威凭据**。要看现状，看 `--service-state` 的 `paused=` / `registered=` 与控制端设备行
