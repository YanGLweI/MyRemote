...
2. 客户端：把 `agent.exe` 拷到目标电脑，**直接双击运行**即打开图形配置界面，填写服务器地址/端口/连接密钥，可点“测试连接并注册”验证；点“保存并后台运行”后 agent 自动转入后台并注册到服务端。仅改配置不启动：`agent.exe --config-ui`。开机自启：`agent.exe --install-autostart`（卸载 `--uninstall-autostart`，自启以 `--background` 静默运行不弹窗）。调试窗口：`agent.exe --console --background`。
...
- 客户端普通进程 + 注册表 `HKCU\...\Run` 自启 + 无控制台窗口（后台静默）
- 不使用 Windows 服务（Session 0 无法捕获交互桌面）
- 客户端仅出站连接，本机不监听任何端口

## 已知限制

- 串流为软件编码，高分辨率（如 3402×1812）下帧率受 GUI 线程 YUV→RGB 与编码开销影响，实测约 15–20fps；可后续接入 GPU 编码/独立解码线程优化
